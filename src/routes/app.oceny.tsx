...
import { GRADE_OPTIONS, gradeToValue } from "@/lib/grade-utils";
...
function GradesPage() {
  ...
  const { data: grades = [] } = useQuery({
    queryKey: ["grades"],
    queryFn: async () => (await supabase.from("grades").select("*, students(first_name, last_name), grade_categories(name), subjects(name)").order("date", { ascending: false }).limit(200)).data ?? [],
  });
  ...
  const add = useMutation({
    ...
    onSuccess: () => {
      toast.success(`Wystawiono ocen: ${selected.length}`);
      qc.invalidateQueries({ queryKey: ["grades"] });
      setOpen(false); setSelected([]); setGrade(""); setDescription("");
    },
    onError: (e: any) => toast.error(e.message),
  });

  const del = useMutation({
    mutationFn: async (id: string) => { await supabase.from("grades").delete().eq("id", id); },
          ...
          <DialogContent className="max-w-2xl">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <Select value={categoryId} onValueChange={(v) => { setCategoryId(v); const c = categories.find(x => x.id === v); if (c) setWeight(String(c.weight)); }}>
                  <SelectTrigger><SelectValue placeholder="Wybierz" /></SelectTrigger>
                  <SelectContent>{categories.map(c => <SelectItem key={c.id} value={c.id}>{c.name} (waga {c.weight})</SelectItem>)}</SelectContent>
                </Select>
              </div>
            ...
            <div>
              ...
              <div className="border rounded-md max-h-48 overflow-y-auto p-2 space-y-1">
                <label className="flex items-center gap-2 px-2 py-1 hover:bg-muted rounded text-sm font-medium border-b mb-1">
                  <Checkbox checked={selected.length === students.length && students.length > 0} onCheckedChange={(v) => setSelected(v ? students.map(s => s.id) : [])} />
                  Wszyscy
                </label>
                {students.map(s => (
            ...
            <tbody>
              {grades.map((g: any) => (
                <tr key={g.id} className="border-t border-border">
                  ...
                  <td className="p-3">{g.students?.last_name} {g.students?.first_name}</td>
              ...
              {!grades.length && <tr><td colSpan={8} className="p-8 text-center text-muted-foreground">Brak ocen.</td></tr>}
            </tbody>
          </table>
        </Card>
      </div>
