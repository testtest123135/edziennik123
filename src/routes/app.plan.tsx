...
const empty = { day_of_week: 1, start_time: "08:00", end_time: "08:45", subject_id: "", class_name: "", room: "" };
...
function SchedulePage() {
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      if (editId) { const { error } = await supabase.from("schedule").update(payload).eq("id", editId); if (error) throw error; }
      else { const { error } = await supabase.from("schedule").insert(payload); if (error) throw error; }
    },
    onSuccess: () => { toast.success(editId ? "Zapisano" : "Dodano"); qc.invalidateQueries({ queryKey: ["schedule"] }); setOpen(false); setEditId(null); setForm(empty); },
  });
  ...
  const openEdit = (i: any) => { setEditId(i.id); setForm({ day_of_week: i.day_of_week, start_time: i.start_time?.slice(0,5) ?? "08:00", end_time: i.end_time?.slice(0,5) ?? "08:45", subject_id: i.subject_id ?? "", class_name: i.class_name ?? "", room: i.room ?? "" }); setOpen(true); };
  return (
    <div>
      <PageHeader title="Plan lekcji" description="Stałe zajęcia w określonych godzinach." actions={
        <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) { setEditId(null); setForm(empty); } }}>
          ...
          <DialogContent>
            <DialogHeader><DialogTitle>{editId ? "Edytuj pozycję" : "Nowa pozycja"}</DialogTitle></DialogHeader>
            <div className="space-y-3">
              ...
              <Button onClick={() => save.mutate()} className="w-full">{editId ? "Zapisz zmiany" : "Dodaj"}</Button>
            </div>
          </DialogContent>
        </Dialog>
      } />
      <div className="p-4 sm:p-6 lg:p-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
              ...
              {(items as any[]).filter(i => i.day_of_week === idx + 1).map(i => (
                <div key={i.id} className="bg-secondary rounded p-2 text-xs">
                  ...
                  <div className="flex gap-2 mt-1">
                    <button onClick={() => openEdit(i)} className="text-primary text-[10px] flex items-center gap-0.5"><Pencil className="w-3 h-3" />edytuj</button>
                    <button onClick={() => del.mutate(i.id)} className="text-destructive text-[10px] flex items-center gap-0.5"><Trash2 className="w-3 h-3" />usuń</button>
                  </div>
                </div>
              ))}
            </div>
